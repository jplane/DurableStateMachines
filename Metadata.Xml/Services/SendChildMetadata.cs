...
namespace StateChartsDotNet.Metadata.Xml.Services
{
    public class SendChildMetadata : SendMessageMetadata
    {
        ...
        internal SendChildMetadata(XElement element)
            : base(element)
        {
            ...
            items.Add(new XAttribute("type", "send-child"));

            if (element.ScxmlElement("child") != null)
            {
                items.Add(new XAttribute("target", element.ScxmlElement("child").Value));
            }
            else if (element.ScxmlElement("childExpr") != null)
            {
                items.Add(new XAttribute("targetexpr", element.ScxmlElement("childExpr").Value));
            }
